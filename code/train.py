...
from rcan import Net as RCAN

# Training parameters
parser = argparse.ArgumentParser(description='RCAN Super-Resolution Settings')
...
parser.add_argument("--nFrames", type=int, default=7, help="Number of input frame to train for")
...
def train(train_loader, scale_factor, epoch_num):
    ...
    for idx_epoch in range(epoch_state, epoch_num):
        for idx_iter, (LR, HR, RF) in enumerate(train_loader):
            LR, HR, RF = Variable(LR).cuda(), Variable(HR).cuda(), Variable(RF).cuda()
            VSR = net(LR, RF)

            loss = criterion_MSE(VSR, HR[:, :, 3, :, :])
